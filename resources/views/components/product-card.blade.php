<section id="produk" 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 lg:gap-6">
            ...
            <div x-show="matches('{{ $prod['category'] }}', '{{ $prod['type_category'] }}')"
                ...
                <div class="p-3 sm:p-4 flex-1 flex flex-col justify-between">
                    ...
                    <div class="pt-2.5 sm:pt-3 border-t border-gray-100 flex items-center justify-between gap-2">
                        ...
                        <button @click="$store.cart.addItem('{{ $prod['id'] }}', '{{ addslashes($prod['name']) }}', {{ $prod['price'] }})" 
                                type="button" 
                                aria-label="Tambahkan {{ $prod['name'] }} ke pesanan"
                                ...
                                :class="$store.cart.lastAddedId === '{{ $prod['id'] }}' ? 'scale-110 bg-emerald-600 ring-2 ring-emerald-400' : ''">
                            
                            <!-- Clean Shopping Cart SVG Icon -->
                            <svg x-show="$store.cart.lastAddedId !== '{{ $prod['id'] }}'" class="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-none stroke-current stroke-2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                            ...
                            <svg x-show="$store.cart.lastAddedId === '{{ $prod['id'] }}'" x-cloak class="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-none stroke-current stroke-2 text-white animate-bounce" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
