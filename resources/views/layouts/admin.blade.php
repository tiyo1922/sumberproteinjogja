@php
    ...
    $adminCustomLogoUrl = $hasAdminCustomLogo
        ? (str_starts_with($rawAdminLogo, 'http') ? $rawAdminLogo : (file_exists(public_path($rawAdminLogo)) ? asset($rawAdminLogo) : asset('storage/' . ltrim($rawAdminLogo, '/'))))
        : null;
    $adminBrandName = $site['brand']['name'] ?? 'Sumber Protein Jogja';
@endphp
<!DOCTYPE html>
...
<body class="h-full bg-[#F4F6F4] text-brand-dark font-sans antialiased selection:bg-brand-primary selection:text-white"
    ...
    <!-- ======================================================= -->
    <!-- 2. DESKTOP & TABLET CMS SHELL (>= 768px)                -->
    <!-- ======================================================= -->
    <div class="hidden md:flex min-h-screen">

        <!-- =================================================== -->
        <!-- 2.1 SIDEBAR NAVIGATION (Dark Charcoal / Slate-900)  -->
        <!-- Width: 64 (256px) on Desktop / Responsive on Tablet -->
        <!-- =================================================== -->
        <aside class="w-64 bg-brand-dark border-r border-brand-dark-soft flex flex-col shrink-0 fixed inset-y-0 left-0 z-40 transition-transform duration-300 ease-in-out lg:translate-x-0 h-screen overflow-hidden"
            ...
            <div class="px-6 pt-3 pb-3.5 flex items-center justify-between border-b border-white/10 shrink-0 bg-brand-dark">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 group">
                    <div class="w-9 h-9 flex items-center justify-center group-hover:scale-105 transition-transform shrink-0">
                        @if($hasAdminCustomLogo)
                            <img src="{{ $adminCustomLogoUrl }}" alt="{{ $adminBrandName }}" class="w-full h-full object-contain">
                        @else
            ...
            <!-- 3. Bottom Sidebar Area: Admin Profile (Pinned Bottom, Natural Flex Flow) -->
            <div class="p-4 border-t border-white/10 bg-black/20 shrink-0">
                <div class="flex items-center justify-between gap-3">
                    <div class="flex items-center gap-3 min-w-0">
                        <div class="w-9 h-9 rounded-full bg-brand-primary text-white flex items-center justify-center text-xs font-bold shrink-0 ring-2 ring-emerald-400/40 overflow-hidden">
                            @php
                        ...
                        <div class="flex flex-col min-w-0">
                            <span class="text-xs font-bold text-white truncate leading-tight admin-user-name-text">{{ auth()->user()->name ?? 'Admin SP Jogja' }}</span>
